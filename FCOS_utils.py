...
distinct_colors = ['#ffffff',' #e6194b', '#3cb44b', '#ffe119', '#0082c8', '#f58231']
...
def create_data_lists_G(G_dataset_path, output_folder):
    """
    Create lists of images, the bounding boxes and labels of the objects in these images, and save these to file.
    ...
    :param output_folder: folder where the JSONs must be saved
    """
    G_dataset_path = os.path.abspath(G_dataset_path)
    #voc12_path = os.path.abspath(voc12_path)
    ...
    for id in ids:
        # Parse annotation's XML file
        objects = parse_annotation_G(os.path.join(G_dataset_path, 'Annotations', id + '.xml'))
        if len(objects['boxes']) == 0:
            continue
        n_objects += len(objects)
        train_objects.append(objects)
        train_images.append(os.path.join(G_dataset_path, 'JPEGImages', id + '.jpg'))
    ...
    # Find IDs of images in the test data
    with open(os.path.join(G_dataset_path, 'ImageSets/Main/test.txt')) as f:
        ids = f.read().splitlines()
    ...
    for id in ids:
        ...
        n_objects += len(objects)
        test_objects.append(objects)
        test_images.append(os.path.join(G_dataset_path, 'JPEGImages', id + '.jpg'))
